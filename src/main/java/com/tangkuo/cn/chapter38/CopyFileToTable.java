package com.tangkuo.cn.chapter38;
...
public class CopyFileToTable extends JFrame {
	...
	// JDBC and table info
	private JComboBox jcboDriver = new JComboBox(new String[] { "com.mysql.jdbc.Driver", "sun.jdbc.odbc.JdbcOdbcDriver",
			"oracle.jdbc.driver.OracleDriver" });
	private JComboBox jcboURL = new JComboBox(new String[] { "jdbc:mysql://localhost/test",
			"jdbc:odbc:exampleMDBDataSource", "jdbc:oracle:thin:@liang.armstrong.edu:1521:orcl" });
	...
	public CopyFileToTable() {
		...
		add(new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, jPanel2, jPanel5), BorderLayout.CENTER);
	...
	private void copyFile() throws Exception {
		...
		Connection conn = DriverManager.getConnection(((String) jcboURL.getSelectedItem()).trim(),
				jtfUsername.getText().trim(), String.valueOf(jtfPassword.getPassword()).trim());
		...
		try {
			...
			try {
				if (connection.getMetaData().supportsBatchUpdates()) {
					...
					System.out.println("The driver is of JDBC 2 type, but " + "does not support batch updates");
				}
